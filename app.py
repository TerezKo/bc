...
def generate_schedule():
    ...
    if 25 <= current_date.day <= 31 and not is_schedule_generated():
        current_year = datetime.now().year
        current_month = datetime.now().month
        month_year = current_year * 100 + current_month
        if current_month == 12:
            next_year = current_year + 1  # Ak sme v decembri, nasledujúci mesiac bude v nasledujúcom roku
            next_month = 1  # Nasledujúci mesiac je január
        else:
            next_year = current_year
            next_month = current_month + 1
        next_month_year = next_year * 100 + next_month
...
def is_schedule_generated():
    ...
    if current_month == 12:
        ...
        next_month = current_month + 1
    month_year = next_month_year * 100 + next_month
...
@app.route('/save_doctor_changes', methods=['POST'])
def save_doctor_changes():
    edited_data = request.json['doctorsData']

    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()

    for i, doctor_data in enumerate(edited_data):
        if i==0:
            pracovisko = "JIS"
        else:
            pracovisko = "Oddelenie"
        pondelok = doctor_data[0]
        utorok = doctor_data[1]
        streda = doctor_data[2]
        stvrtok = doctor_data[3]
        piatok = doctor_data[4]

        cursor.execute("UPDATE lekari SET pondelok = ?, utorok = ?, streda = ?, stvrtok = ?, piatok = ? WHERE pracovisko = ?",
                       (pondelok, utorok, streda, stvrtok, piatok, pracovisko))

    conn.commit()
    cursor.close()
    conn.close()

    return jsonify({"message": "Zmeny úspešne uložené"})
...
def admin():
    ...
    cursor.execute("SELECT * FROM lekari")
    ...
    return render_template('admin.html', employees=employees, message=message, doctors=doctors)
...
def employee():
    ...
    cursor.execute("SELECT MAX(month_year) FROM schedules")
    month_year = cursor.fetchone()[0]
    web_month_year = current_year * 100 + current_month
    ...
    if month_year != web_month_year:
        preferences_submitted = "Po termíne"
    cursor.execute("SELECT position FROM users WHERE id = ?", (user_id,))
    position = cursor.fetchone()[0]

    return render_template('employee.html', preferences_submitted=preferences_submitted, position=position)
